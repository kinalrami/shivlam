...
import { attachBrandHeroCanvas } from "./brandHeroCanvas";

type Props = {
  content: BrandBuildingContent["hero"];
};

export function Hero({ content }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [chipTop, setChipTop] = useState("↑ RANK #1 · GOOGLE");
  const [chipBot, setChipBot] = useState("● BRAND GROWING");

  useEffect(() => {
    const c = canvasRef.current;
    if (!c) return;
    return attachBrandHeroCanvas(c, (v) => {
      setChipTop(v.top);
      setChipBot(v.bot);
    });
  }, []);
  ...
  return (
    <section id="hero" className="relative flex min-h-screen items-center overflow-hidden bg-[#060606] scroll-mt-14">
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          backgroundImage:
            "linear-gradient(rgba(255,153,51,.08) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.06) 1px, transparent 1px)",
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-325 grid-cols-1 items-center gap-14 px-5 pt-[clamp(4.5rem,10vh,6.5rem)] pb-16 md:grid-cols-2 md:gap-16 md:px-12">
        <div className="min-w-0">
          ...
          <h1 className="mb-5 max-w-full break-words font-sans text-[clamp(1.9rem,4.6vw,4rem)] leading-[0.96] font-extrabold tracking-[-0.035em] text-white">
            {content.titleBefore} <span className="text-orange-400">{content.titleHighlight}</span> {content.titleAfter}
          </h1>
        ...
        <div className="relative flex items-center justify-center">
          <div className="relative w-full max-w-[560px]">
            ...
            <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#04081a] shadow-[0_40px_90px_rgb(0_0_0/0.55)]">
              <canvas ref={canvasRef} className="block h-[420px] w-full min-w-0 max-w-full" />
            </div>
          </div>
        </div>
      </div>
