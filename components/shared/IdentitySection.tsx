...
export type IdentityStat = { value: string; label: string };
...
type Props = {
  ...
  /** Optionally override the CTA block entirely. */
  actions?: ReactNode;
  /** Optionally override stats under the seal. */
      ...
      <div className="relative z-10 mx-auto max-w-325 px-5 md:px-12">
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <ArbimReveal delayStep={3} className="mt-10 flex flex-wrap gap-3.5">
              {actions ??
                (primaryCta || secondaryCta ? (
                  <>
                    {primaryCta ? (
                      ...
                      >
                        {primaryCta.label} <MoveRight className="size-4" />
                      </Link>
                    ) : null}
                    {secondaryCta ? (
                      <Link
          ...
          <div className="relative flex flex-col items-center gap-8">
            ...
            <ArbimReveal
              delayStep={3}
              className="grid w-full max-w-sm grid-cols-1 gap-3 sm:grid-cols-3 lg:max-w-none"
            >
              {stats.map((s) => (
                <div
                  key={s.label}
                ...
                >
                  <div className="font-sans text-2xl font-extrabold text-orange-400">
                    {s.value}
                  </div>
